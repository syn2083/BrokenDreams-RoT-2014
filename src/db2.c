...
/*
 * Snarf a mob section.  new stance
 */
void load_mobiles( FILE *fp )
{
    ...
    for ( ; ; )
    {
		...
		// pMobIndex->exbit1                 = fread_flag( fp ) 
		// | race_table[pMobIndex->race].exbit1;
	...
	for ( ; ; )
		...
		if (pMobIndex->clan)
		{
		    bug("Load_mobiles: duplicate clan fields.",0);
		    exit(1);
		}
...
/*
 * Snarf an obj section. new stance
 */
	...
	switch (letter)
 	{
	   ...
	   else if ( letter == 'G') /* guild */
	   {
		...
		if (guild->guild != -1)
		{
		    ...
		    bug("Load_objects: vnum %d has an unknown guild name.",vnum);
		}
	    }
		else if ( letter == 'O' )
	    {
...
void load_dreams( FILE *fp )
{
	...
	vnum = top_drm_index;
        ...
        iHash                   = vnum % MAX_KEY_HASH;
        pDrmIndex->next         = drm_index_hash[iHash];
        drm_index_hash[iHash]   = pDrmIndex;
        top_drm_index++;
    }
 ...
 {
     int level;
     unsigned long number, type;  /* for dice-conversion */
