...
/* returns a flag for levnet */
long levnet_lookup (const char *name)
{
    int flag;

    for (flag = 0; levnet_table[flag].name != NULL; flag++)
    {
	if (LOWER(name[0]) == LOWER(levnet_table[flag].name[0])
	&& !str_prefix(name,levnet_table[flag].name))
	    return flag;
    }
...
int pk_range(CHAR_DATA *ch,CHAR_DATA *victim)
{
/*
	0 = no pkill
	1 = ch attack victim only
	2 = common attack
	3 = victim attack ch only
*/
    ...
    if (ch->on_gquest || victim->on_gquest)
	return 0;
...
bool pk_rangerev(CHAR_DATA *ch,CHAR_DATA *victim)
{
    if (IS_NPC(ch) || IS_NPC(victim))
	return FALSE;
    ...
    if (IS_SET(victim->exbit1_flags, PK_LAWFUL) 
...
bool class_can_use(CHAR_DATA *ch, OBJ_DATA *obj)
{
	...
	if (ch->class == guild->guild)
	    return TRUE;
	if (!IS_NPC(ch) && (ch->pcdata->tier == 2) && (ch->clasb == guild->guild) )
	    return TRUE;
    }
    return FALSE;
}
...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	|| IS_SET(ch->act,ACT_RANGER) || IS_SET(ch->act,ACT_WARRIOR)))
	    ...
	    skill = 20 + 3 * ch->level;
	else if (sn == gsn_fade && IS_SET(ch->off_flags,OFF_FADE))
	    skill = 20 + 2 * ch->level;
	else if (sn == gsn_trip && IS_SET(ch->off_flags,OFF_TRIP))
	    skill = 20 + 3 * ch->level;
...
int get_curr_stat( CHAR_DATA *ch, int stat )
{
	...
	if ( (ch->pcdata->tier == 2) && (class_table[ch->clasb].attr_prime == stat) )
	    max += 1;
...
/*
 * Find a char in the world. Name must match exact.
 */
CHAR_DATA *get_exact_world( CHAR_DATA *ch, char *argument )
{
    char arg[MIL];
    CHAR_DATA *wch;
    int number;
...
OBJ_DATA *create_money( int platinum, int gold, int silver )
{
    ...
    {
	bug( "Create_money: negative money.",UMIN(UMIN(platinum,gold),silver));
	platinum = UMAX(0,platinum);
	gold = UMAX(0,gold);
...
/* visibility on an exit */
bool can_see_door( CHAR_DATA *ch, long exit_info )
{
    if (IS_NPC(ch))
	return TRUE;
    ...
    if (IS_SET(exit_info, EX_MAGIC) 
    &&  !IS_AFFECTED(ch, AFF_DETECT_MAGIC))
	return FALSE;

    if (IS_SET(exit_info, EX_INVIS) 
    &&  !IS_AFFECTED(ch, AFF_DETECT_INVIS))
	return FALSE;

    if (IS_SET(exit_info, EX_FLY) 
    &&  !IS_AFFECTED(ch, AFF_FLYING))
	return FALSE;

    if (IS_SET(exit_info, EX_SWIM) 
    &&  !IS_AFFECTED(ch, AFF_FLYING))
    {
	OBJ_DATA *obj;
	bool found;
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    &&   !can_see_door(ch, (long)obj->value[1]) )
	return FALSE;
...
bool can_drop_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	&& !IS_SET(obj->extra_flags, ITEM_LQUEST) )
	return TRUE;
...
char *item_type_name( OBJ_INDEX_DATA *obj )
{
    ...
    case ITEM_PASSBOOK:		return "passbook";
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_LQUEST	 ) strcat( buf, " levelquest"	);
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *act2_bit_name( int act2_flags )
{
    ...
    if (IS_SET(act2_flags,ACT2_IS_NPC))
    { 
 	strcat(buf," npc");
    if (act2_flags & ACT2_FORGER      ) strcat(buf, " forger");
    }
    else
    {
	strcat(buf," player");
...
char *form_bit_name(int form_flags)
{
    ...
    else if (form_flags && FORM_MAGICAL	) { strcat(buf, " magical"); }
    else if (form_flags && FORM_INSTANT_DECAY	) { strcat(buf, " instant_rot"); }
    else if (form_flags && FORM_OTHER		) { strcat(buf, " other"); }
    else if (form_flags && FORM_ANIMAL	) { strcat(buf, " animal"); }
    else if (form_flags && FORM_SENTIENT	) { strcat(buf, " sentient"); }
    else if (form_flags && FORM_UNDEAD	) { strcat(buf, " undead"); }
    else if (form_flags && FORM_CONSTRUCT	) { strcat(buf, " construct"); }
    else if (form_flags && FORM_MIST		) { strcat(buf, " mist"); }
    else if (form_flags && FORM_INTANGIBLE	) { strcat(buf, " intangible"); }
    else if (form_flags && FORM_BIPED		) { strcat(buf, " biped"); }
    else if (form_flags && FORM_CENTAUR	) { strcat(buf, " centaur"); }
    else if (form_flags && FORM_INSECT	) { strcat(buf, " insect"); }
    else if (form_flags && FORM_SPIDER	) { strcat(buf, " spider"); }
    else if (form_flags && FORM_CRUSTACEAN	) { strcat(buf, " crustacean"); }
    else if (form_flags && FORM_WORM		) { strcat(buf, " worm"); }
    else if (form_flags && FORM_BLOB		) { strcat(buf, " blob"); }
    else if (form_flags && FORM_MAMMAL	) { strcat(buf, " mammal"); }
    else if (form_flags && FORM_BIRD		) { strcat(buf, " bird"); }
    else if (form_flags && FORM_REPTILE	) { strcat(buf, " reptile"); }
    else if (form_flags && FORM_SNAKE		) { strcat(buf, " snake"); }
    else if (form_flags && FORM_DRAGON	) { strcat(buf, " dragon"); }
    else if (form_flags && FORM_AMPHIBIAN	) { strcat(buf, " amphibian"); }
    else if (form_flags && FORM_FISH		) { strcat(buf, " fish"); }
    else if (form_flags && FORM_COLD_BLOOD 	) { strcat(buf, " cold_blooded"); }

/*
edible poison magical instant_decay other animal sentient undead construct mist biped spider crustacean
*/
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
