...
bool				global_gquest	= FALSE;
bool				global_xpq	= FALSE;
bool				global_prq	= FALSE;
bool				global_damq	= FALSE;
...
const	struct	cmd_type	cmd_table	[] =
{
    ...
    { "guild",		do_guild,	POS_DEAD,	L3,  1,  LOG_ALWAYS, 1 },
    { "hit",		do_kill,	POS_FIGHTING,	 0,  1,  LOG_NORMAL, 0 },
    { "camp",           do_camp,        POS_STANDING,    0,  1,  LOG_NORMAL, 1 },
    ...
    { "study",       	do_study,	POS_SLEEPING,    0,  1,  LOG_NORMAL, 1 },
    { "rest",		do_rest,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 },
    { "run",		do_run,		POS_STANDING,	 0,  1,  LOG_NEVER,  1 },
    ...
    { "wizhelp",	do_wizhelp,	POS_DEAD,	IM,  1,  LOG_NORMAL, 1 },
...
/*  { "groups",		do_groups,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 }, */
    ...
    { "bskills",	do_bskills,	POS_DEAD,	 0,  1,  LOG_NORMAL, 1 },
...
/*  { "channels",	do_channels,	POS_DEAD,	 0,  1,  LOG_NORMAL, 1 }, */
    ...
    { "titlelock",	do_titlelock,	POS_DEAD,	 0,  1,  LOG_NORMAL, 1 },
...
// { "arena",		do_arena,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 },
   { "gocial",		do_gocial,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 },
// { "=",		do_gocial,     	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 0 },
   { "gmote",		do_gmote,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 },
// { "flame ",		do_flame,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 },
   { "gquest",		do_gquest,	POS_RESTING,	 0,  1,  LOG_NORMAL, 1 },
   { "drive",		do_drive,	POS_SITTING,	 0,  1,  LOG_NORMAL, 1 },
// { "5", 		do_5,		POS_FIGHTING,    0,  1,  LOG_NORMAL, 1 },
   { "stance",		do_stance,	POS_FIGHTING,	 0,  1,  LOG_NORMAL, 1 },
// { "call",		do_call,	POS_STANDING,    0,  1,  LOG_NORMAL, 1 },
// { "claim",		do_claim,	POS_STANDING,    0,  1,  LOG_NORMAL, 1 },
   { "locate",		do_locate,	POS_STANDING,    0,  1,  LOG_NORMAL, 1 },
   { "showclass",	do_showclass,	POS_STANDING,    0,  1,  LOG_NORMAL, 1 },
   { "autostance", 	do_autostance, 	POS_STANDING,	 0,  1,  LOG_NORMAL, 1 },
   { "leave", 		do_leave, 	POS_STANDING,	 0,  1,  LOG_NORMAL, 1 },
   { "jab",		do_jab,		POS_FIGHTING,    0,  1,  LOG_NORMAL, 1 },
   { "uppercut",        do_uppercut,    POS_FIGHTING,    0,  1,  LOG_NORMAL, 1 },
   { "right cross",	do_right_cross, POS_FIGHTING,    0,  1,  LOG_NORMAL, 1 },
   { "critical strike",	do_critical_strike,	POS_FIGHTING,    0,  1,  LOG_NORMAL, 1 },
   { "kidney punch",	do_kidney_punch,	POS_FIGHTING,    0,  1,  LOG_NORMAL, 1 },
   { "left hook",	do_left_hook,	POS_FIGHTING,	 0,  1,  LOG_NORMAL, 1 },
...
/*  { "music",		do_music,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 }, */
    { "mud",		do_mud,		POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 },
    ...
    { "aquest",		do_aquest,	POS_RESTING,	 0,  1,  LOG_NORMAL, 1 },
    ...
    { "inscribe",	do_inscribe,	POS_RESTING,	HE,  1,  LOG_NORMAL, 1 }, 
    ...
    { "recite",		do_recite,	POS_RESTING,	 102,  1,  LOG_NORMAL, 1 },
    { "remove",		do_remove,	POS_RESTING,	 0,  1,  LOG_NORMAL, 1 },
    { "rob",		do_rob,		POS_STANDING,	 0,  1,  LOG_NORMAL, 1 },
    { "route",		do_route,	POS_STANDING,	 0,  1,  LOG_NORMAL, 1 },
...
/*  { "unlock",		do_unlock,	POS_RESTING,	 0,  1,  LOG_NORMAL, 1 }, */
    ...
    { "decline",        do_decline,     POS_STANDING,    0,  1,  LOG_NORMAL, 1 },
    { "bet",            do_bet,         POS_RESTING,     0,  1,  LOG_NORMAL, 1 },
    ...
    { "tail",		do_tail,	POS_FIGHTING,	 0,  1,  LOG_NORMAL, 1 },
...
/*  { "group",		do_group,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 }, */
    ...
    { "enlist",		do_enlist,		POS_DEAD,	12,  1,  LOG_NORMAL, 1 },
...
/*  { "practice",	do_practice,	POS_SLEEPING,	 0,  1,  LOG_NORMAL, 1 }, */
     ...
     * Deity commands.
     */
    { "devote",		do_devote,	POS_STANDING,	10,  1,  LOG_NORMAL, 1 },
    { "supplicate",	do_supplicate,	POS_STANDING,	10,  1,  LOG_NORMAL, 1 },
    { "deity",		do_deities,	POS_STANDING,	10,  1,  LOG_NORMAL, 1 },
...
/*  { "sockets",	do_sockets,	POS_DEAD,	IM,  1,  LOG_NORMAL, 1 }, */
    ...
    { "pack",		do_pack,	POS_DEAD,	L7,  1,  LOG_NORMAL, 1 },
    { "levnet",		do_levnet,	POS_DEAD,	L7,  1,  LOG_NORMAL, 1 },
    { "pecho",		do_pecho,	POS_DEAD,	L5,  1,  LOG_NORMAL, 1 }, 
    { "purge",		do_purge,	POS_DEAD,	IM,  1,  LOG_NORMAL, 1 },
    { "restore",	do_restore,	POS_DEAD,	L6,  1,  LOG_NORMAL, 1 },
...
//  { "match",		do_arena,	POS_DEAD,	IM,  1,  LOG_NORMAL, 1 },
    { "twit",		do_twit,	POS_DEAD,	L7,  1,  LOG_NORMAL, 1 },
    { "pardon",		do_pardon,	POS_DEAD,	L7,  1,  LOG_NORMAL, 1 },
	{ "relevel",	do_relevel,	POS_DEAD,	1, 1, LOG_ALWAYS, 1},
	{ "winfo",		do_winfo,		POS_DEAD, 	L3, 1, LOG_ALWAYS, 1},
	{ "cprime",	do_cprime,	POS_DEAD,	L8, 1, LOG_ALWAYS, 1 },
	{ "trackbuffer",	do_trackbuffer, POS_DEAD, L8, 1, LOG_ALWAYS, 1},
	{ "resetarea", do_resetarea,		POS_DEAD, L8, 1, LOG_ALWAYS, 1},
	{ "forcetick", 	do_tick,			POS_DEAD, ML, 1, LOG_ALWAYS, 1},
...
/*  { "at",		do_at,		POS_DEAD,	L6,  1,  LOG_NORMAL, 1 }, */
    ...
    { "dm",		do_dmecho,	POS_DEAD,	ML,  1,  LOG_NORMAL, 1 },
/*  { "goto",		do_goto,	POS_DEAD,	L8,  1,  LOG_NORMAL, 1 }, */
    ...
    { "olevel",         do_olevel,      POS_DEAD,       L6,  1,  LOG_ALWAYS, 1 },
    { "mlevel",         do_mlevel,      POS_DEAD,       L6,  1,  LOG_ALWAYS, 1 },
    ...
    { "wiznet",		do_wiznet,	POS_DEAD,	IM,  1,  LOG_NORMAL, 1 },
    ...
    { "pload",          do_pload,   POS_STANDING,       L1,  1,  LOG_NORMAL, 1 },
    { "punload",        do_punload, POS_STANDING,       L1,  1,  LOG_NORMAL, 1 },
	{ "bhelp",			do_bhelp, 	POS_DEAD, 0, 1, LOG_NORMAL, 1},
	{ "sqlgen",		do_sqlgenselect, POS_DEAD, L8, LOG_ALWAYS, 1},
	{ "project",		do_project,	POS_DEAD, L8, LOG_ALWAYS, 1 },

    /*
     * OLC
     */
    { "edit",           do_olc,         POS_DEAD,       L5,  1,  LOG_NORMAL, 1 },
    { "asave",          do_asave,       POS_DEAD,       L5,  1,  LOG_NORMAL, 1 },
    { "alist",          do_alist,       POS_DEAD,       IM,  1,  LOG_NORMAL, 1 },
    { "resets",         do_resets,      POS_DEAD,    	L5,  1,  LOG_NORMAL, 1 },
    { "redit",          do_redit,       POS_DEAD,    	L5,  1,  LOG_NORMAL, 1 },
    { "medit",          do_medit,       POS_DEAD,    	L5,  1,  LOG_NORMAL, 1 },
    { "aedit",          do_aedit,       POS_DEAD,    	ML,  1,  LOG_NORMAL, 1 },
    { "oedit",          do_oedit,       POS_DEAD,    	L5,  1,  LOG_NORMAL, 1 },
    { "mpedit",         do_mpedit,      POS_DEAD,       L5,  1,  LOG_NORMAL, 1 },
    { "hedit",          do_hedit,       POS_DEAD,       L1,  1,  LOG_NORMAL, 1 },

    /*
     * Commands needed last on list
     */
    { "lone",		do_lone,	POS_DEAD,	15,  1,  LOG_NORMAL, 0 },
    { "loner",		do_loner,	POS_STANDING,	15,  1,  LOG_NORMAL, 1 },
    ...
    { "reroll",		do_reroll,	POS_STANDING,	 1,  1,  LOG_NORMAL, 1 },
...
void interpret( CHAR_DATA *ch, char *argument )
{
	...
	char cmds_copy[MAX_INPUT_LENGTH] ;
	...
	&&   cmd_table[cmd].level <= trust )
	{
	    ...
	    if ((cmd_table[cmd].tier == 2) && ((ch->pcdata->tier >= 1)
	    ||  (IS_HERO(ch))))
	    {
		found = TRUE;
		break;
	    }
	    if ((cmd_table[cmd].tier == 3) && (ch->pcdata->tier == 2))
	    {
		found = TRUE;
		break;
	    }
	    if ((cmd_table[cmd].tier == 3) && (ch->pcdata->tier == 1)
	    &&  (IS_HERO(ch)))
	    {
		found = TRUE;
		break;
	...
	{
		sprintf(bufs,
		"Memcheck : Increase in strings :: %s : %s", ch->name, cmds_copy) ;
		wiznet(bufs, NULL, NULL, WIZ_MEMCHECK,0,0) ;
	}

	if (perm_count < nAllocPerm)
	{
		sprintf(bufs,
		"Increase in perms :: %s : %s", ch->name, cmds_copy) ;
		wiznet(bufs, NULL, NULL, WIZ_MEMCHECK, 0,0) ;
	}
    tail_chain( );
    return;
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
	...
	if (!str_cmp(race_table[victim->race].name, "centipede"))
                {
                        act( "$n looks as though its been squashed by $N.",  victim, NULL, ch, TO_NOTVICT );
                        act( "You feel as though you've been squashed by $N.",  victim, NULL, ch, TO_CHAR    );
                        act( "$n looks as though its been squashed by you.", victim, NULL, ch, TO_VICT    );
                        break;
                }
                else
                {
	...
	case 9: if (!str_cmp(race_table[victim->race].name, "centipede"))
                {
                        act( "$n crawls away from $N.",  victim, NULL, ch, TO_NOTVICT );
                        act( "You crawl away from $N.",  victim, NULL, ch, TO_CHAR    );
                        act( "$n crawls away from you.", victim, NULL, ch, TO_VICT    );
                        break;
                }
                else
                {
	     ...
	     case 10: case 11: 
	case 12: if (!str_cmp(race_table[victim->race].name, "centipede"))
                {
                        act( "$n tries to bite $N.",  victim, NULL, ch, TO_NOTVICT );
                        act( "You try to bite $N.",  victim, NULL, ch, TO_CHAR    );
                        act( "$n tries to bite you.", victim, NULL, ch, TO_VICT    );
                        break;
                }
                else
                {
...
void do_commands( CHAR_DATA *ch, char *argument )
{
	...
	&&   cmd_table[cmd].show)
	{
		...
		if ( ++col % 6 == 0 )
		    send_to_char( "\n\r", ch );
	    } else if (cmd_table[cmd].tier == 1)
	    {
		sprintf( buf, "%-12s", cmd_table[cmd].name );
		send_to_char( buf, ch );
		if ( ++col % 6 == 0 )
		    send_to_char( "\n\r", ch );
	    } else if ((cmd_table[cmd].tier == 2) && ((ch->pcdata->tier >= 1)
	    ||  (IS_HERO(ch))))
	    {
		sprintf( buf, "%-12s", cmd_table[cmd].name );
		send_to_char( buf, ch );
		if ( ++col % 6 == 0 )
		    send_to_char( "\n\r", ch );
	    } else if ((cmd_table[cmd].tier == 3) && (ch->pcdata->tier == 2))
	    {
		sprintf( buf, "%-12s", cmd_table[cmd].name );
		send_to_char( buf, ch );
		if ( ++col % 6 == 0 )
		    send_to_char( "\n\r", ch );
	    } else if ((cmd_table[cmd].tier == 3) && (ch->pcdata->tier == 1)
	    &&  (IS_HERO(ch)))
	    {
		sprintf( buf, "%-12s", cmd_table[cmd].name );
		send_to_char( buf, ch );
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
    ...
    for( clevel=MAX_LEVEL; clevel>0; clevel-- )
    {
      ...
      case ML:
              ...
              break;
      case L1:
              if (ch->level >= CREATOR || ch->trust == CREATOR)
              {
              send_to_char("\n\r{D--{cLevel 109{D--{X\n\r", ch);
              }
              break;
      case L2:
              ...
              break;
      case L3:
              if (ch->level >= DEITY || ch->trust == DEITY)
              {
              send_to_char("\n\r{D--{cLevel 107{D--{X\n\r", ch);
              }
              break;
      ...
      case L5:
              ...
              break;
      case L6:
              if (ch->level >= DEMI || ch->trust == DEMI)
              {
              send_to_char("\n\r{D--{cLevel 104{D--{X\n\r", ch);
              }
              break;
      case L7:
              if (ch->level >= KNIGHT || ch->trust == KNIGHT)
              {
              send_to_char("\n\r{D--{cLevel 103{D--{X\n\r", ch);
              }
              break;
      ...
      case L8:
              if (ch->level >= SQUIRE || ch->trust == SQUIRE)
              {
              send_to_char("\n\r{D--{cLevel 102{D--{X\n\r", ch);
              }
              break;
