 ...
 * Local functions.
 */
AREA_DATA *get_area_data	args( ( unsigned long vnum ) );
...
char *olc_ed_vnum( CHAR_DATA *ch )
{
	...
	sprintf( buf, "%lu", pArea ? pArea->vnum : 0 );
	break;
    case ED_ROOM:
	pRoom = ch->in_room;
	sprintf( buf, "%lu", pRoom ? pRoom->vnum : 0 );
	break;
    case ED_OBJECT:
	pObj = (OBJ_INDEX_DATA *)ch->desc->pEdit;
	sprintf( buf, "%lu", pObj ? pObj->vnum : 0 );
	break;
    case ED_MOBILE:
	pMob = (MOB_INDEX_DATA *)ch->desc->pEdit;
	sprintf( buf, "%lu", pMob ? pMob->vnum : 0 );
	break;
    case ED_MPCODE:
    	pMprog = (PROG_CODE *)ch->desc->pEdit;
    	sprintf( buf, "%lu", pMprog ? pMprog->vnum : 0 );
	break;
	case ED_OPCODE:
	pOprog = (PROG_CODE *)ch->desc->pEdit;
	sprintf( buf, "%lu", pOprog ? pOprog->vnum : 0 );
	break;
    case ED_RPCODE:
	pRprog = (PROG_CODE *)ch->desc->pEdit;
	sprintf( buf, "%lu", pRprog ? pRprog->vnum : 0 );
	break;
    default:
	sprintf( buf, " " );
	break;
 ...
 ****************************************************************************/
AREA_DATA *get_area_data( unsigned long vnum )
{
    AREA_DATA *pArea;
...
void display_resets( CHAR_DATA *ch )
{
  ...
  "\n\r", ch );
	...
	case 'M':
	    ...
	    {
                sprintf( buf, "Load Mobile - Bad Mob %lu\n\r", pReset->arg1 );
				log_string(LOG_OLC, buf);
                strcat( final, buf );
                continue;
	    }
	    ...
	    {
                sprintf( buf, "Load Mobile - Bad Room %lu\n\r", pReset->arg3 );
				log_string(LOG_OLC, buf);
                strcat( final, buf );
                continue;
	    }
            ...
            pMob = pMobIndex;
            sprintf( buf, "M[%5lu] %-13.13s in room             R[%5lu] %2d-%2d %-15.15s\n\r{x",
	...
	case 'O':
	    ...
	    {
                sprintf( buf, "Load Object - Bad Object %lu\n\r",
		    pReset->arg1 );
			log_string(LOG_OLC, buf);
                strcat( final, buf );
                continue;
	    ...
	    if ( !( pRoomIndex = get_room_index( pReset->arg3 ) ) )
	    {
                sprintf( buf, "Load Object - Bad Room %lu\n\r", pReset->arg3 );
                strcat( final, buf );
				log_string(LOG_OLC, buf);
                continue;
	    }

            sprintf( buf, "O[%5lu] %-13.13s in room             "
                          "R[%5lu]       %-15.15s\n\r{x",
	...
	case 'P':
	    if ( !( pObjIndex = get_obj_index( pReset->arg1 ) ) )
	    {
                sprintf( buf, "Put Object - Bad Object %lu\n\r",
                    pReset->arg1 );
					log_string(LOG_OLC, buf);
                strcat( final, buf );
                continue;
	    ...
	    {
                sprintf( buf, "Put Object - Bad To Object %lu\n\r",
                    pReset->arg3 );
					log_string(LOG_OLC, buf);
                strcat( final, buf );
                continue;
	    ...
	    sprintf( buf,
		"O[%5lu] %-13.13s inside              O[%5lu] %2d-%2d %-15.15s\n\r",
		pReset->arg1,
		pObj->short_descr,
		pReset->arg3,
		pReset->arg2,
	...
	case 'E':
	    if ( !( pObjIndex = get_obj_index( pReset->arg1 ) ) )
	    {
                sprintf( buf, "Give/Equip Object - Bad Object %lu\n\r",
                    pReset->arg1 );
                strcat( final, buf );
                continue;
	    }
	    ...
	    {
	    sprintf( buf,
		"O[%5lu] %-13.13s in the inventory of S[%5lu]       %-15.15s\n\r",
		pReset->arg1,
		pObj->short_descr,                           
		pMob->vnum,
		pMob->short_descr  );
	    ...
	    sprintf( buf,
		"O[%5lu] %-13.13s %-19.19s M[%5lu]       %-15.15s\n\r",
		pReset->arg1,
		pObj->short_descr,
		(pReset->command == 'G') ?
	...
	case 'D':
	    ...
	    sprintf( buf, "R[%5lu] %s door of %-19.19s reset to %s\n\r",
		pReset->arg1,
	...
	case 'R':
	    ...
	    {
		sprintf( buf, "Randomize Exits - Bad Room %lu\n\r",
		    pReset->arg1 );
		strcat( final, buf );
		continue;
	    }

	    sprintf( buf, "R[%5lu] Exits are randomized in %s\n\r",
...
/*****************************************************************************
 ...
 Called by:	interpreter(interp.c) -- fixed, now it wont freeze the mud! - Syn
 ****************************************************************************/
void do_alist( CHAR_DATA *ch, char *argument )
{
	...
	bufferz = new_buf(1000);
    sprintf( buf1, "[%3s] [%-27s] (%-5s-%5s) [%-10s] %3s [%-10s]\n\r", "Num", "Area Name", "lvnum", "uvnum", "Filename", "Sec", "Builders" );
	send_to_char(buf1,ch);
    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
	sprintf( buf, "[%3lu] %-29.29s (%-5lu-%5lu) %-12.12s [%d] [%-10.10s]\n\r",
	     pArea->vnum,
	     pArea->name,
	     pArea->min_vnum,
	     pArea->max_vnum,
