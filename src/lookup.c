...
int home_lookup(const char * name)
{
    int home;

    for (home = 0; home_table[home].name != NULL; home++)
    {
	if (LOWER(name[0]) == LOWER(home_table[home].name[0])
	&&  !str_prefix(name,home_table[home].name))
	    return home;
    }
