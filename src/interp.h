...
#define L7	MAX_LEVEL - 7	/* knight */
#define L8	MAX_LEVEL - 8	/* squire */
#define IM	LEVEL_IMMORTAL 	/* squire */
...
struct	cmd_type
{
    char * const	name;	    /* command entered */
    DO_FUN *		do_fun;	    /* the do function */
    sh_int		position;   /* minimum required position */
    sh_int		level;	    /* minimum required level of char */
    sh_int		tier;	    /* which tier? 1, 2, or 3 */
    sh_int		log;	    /* log type */
    sh_int              show;	    /* show in commands? 1=yes 0=no */
};
...
DECLARE_DO_FUN( do_xpq	);
DECLARE_DO_FUN( do_prq	);
DECLARE_DO_FUN( do_damq	);
...
DECLARE_DO_FUN(	do_left_hook	);
DECLARE_DO_FUN(	do_levnet	);
...
DECLARE_DO_FUN(	do_right_cross	);
DECLARE_DO_FUN(	do_rob		);
DECLARE_DO_FUN(	do_route	);
...
DECLARE_DO_FUN(do_sqlgenselect);
DECLARE_DO_FUN(do_relevel);
DECLARE_DO_FUN(do_winfo);
