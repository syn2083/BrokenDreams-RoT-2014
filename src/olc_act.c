 ...
 ****************************************************************************/
bool check_range( unsigned long lower, unsigned long upper )
{
    AREA_DATA *pArea;
    int cnt = 0;
...
AREA_DATA *get_vnum_area( unsigned long vnum )
{
    AREA_DATA *pArea;
...
AEDIT( aedit_vnum )
{
    ...
    unsigned long  ilower;
    unsigned long  iupper;
...
REDIT( redit_mreset )
{
    ...
    sprintf( output, "%s (%lu) has been loaded and added to resets.\n\r"
...
REDIT( redit_oreset )
{
	...
	sprintf( output, "%s (%lu) has been loaded into "
	    "%s (%lu) and added to resets.\n\r",
	...
	sprintf( output, "%s (%lu) has been loaded "
	    "%s of %s (%lu) and added to resets.\n\r",
	    capitalize( pObjIndex->short_descr ),
	    pObjIndex->vnum,
...
MEDIT( medit_show )
{
    ...
    sprintf( buf, "Vnum:        [%5lu] Sex:   [%s]   Race: [%s]\n\r",
	pMob->vnum,
...
MEDIT( medit_damtype )
{
	...
	send_to_char( "For a list of message types, ? weapon\n\r", ch );
	return FALSE;
    }
...
MEDIT( medit_shop )
{
	...
	if ( ( value = flag_value( type_flags, argument ) ) == NO_FLAG )
	{
	    send_to_char( "MEdit:  Unknown object type.\n\r", ch );
	    return FALSE;
	}
...
REDIT (redit_copy)
{
	...
	if (cAll)
    {
      sprintf(buf,  "\r\n{rREDIT: {yCOPY: Room %lu duplicated.{x\r\n", vnum );
	  send_to_char (buf, ch);
      return TRUE;
    }
...
OEDIT (oedit_copy)
{
	...
	if (cAll)
    {
      sprintf(buf,  "\r\n{rOEDIT: {yCOPY: Object %lu duplicated.{x\r\n", vnum );
	  send_to_char (buf, ch);
      return TRUE;
    }
...
// Straight copy, just let them know it all was.
	if (cAll)
    {
      sprintf(buf,  "\r\n{rMEDIT: {yCOPY: %s (Mob %lu) duplicated.{x\r\n", pMobOrig->short_descr, vnum );
	  send_to_char (buf, ch);
      return TRUE;
    }
