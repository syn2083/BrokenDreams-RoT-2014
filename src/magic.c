...
case IS_RESISTANT:	save += 1.5;	break;
case IS_VULNERABLE:	save -= 2.2;	break;
    }
	...
	if (class_table[victim->class].fMana)
	    ...
	    if (class_table[victim->clasb].fMana)
	    {
		save -= 1;
		save = 9 * save / 10;
	    }
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->spirit)
    {
        send_to_char( "That's hard to do when you're dead.\n\r", ch);
        return;
    }
	...
	if ( arg2[0] == '\0' )
	{
		...
		send_to_char( "They would notice that.\n\r", ch );
		return;
	    }
	}
	else
	{
		...
		for (door=0;door<13;door++)
		{
		    if ((pExit = ch->in_room->exit[door]) != NULL)
		    {
			for (victim=pExit->u1.to_room->people; victim != NULL; victim=victim->next_in_room)
			{
			    if (!IS_NPC(victim) && victim->invis_level > get_trust(ch)) continue;
			    if ( get_trust(ch) < victim->ghost_level) continue;
			    if (!can_see(ch, victim)) continue;
			    if (is_name( target_name, victim->name ) )
			    {
				found = TRUE;
				break;
			    }
		...
		{
		    send_to_char( "Noone by that name around here.\n\r", ch );
		    return;
		}
	    }
	}
	...
	else if ((ch->pcdata->tier == 2) && (class_table[ch->clasb].fMana))
	{
             (*skill_table[sn].spell_fun) ( sn, ch->level, ch, vo,target);
	}
	else
...
void spell_cause_light( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if (global_damq){ 
	    damage( ch, victim, dice(4, 16) + level / 3, sn,DAM_HARM,TRUE);
	} else {
...
void spell_cause_critical(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
	...
	if (global_damq){ 
    damage( ch, victim, dice(6, 16) + level - 6, sn,DAM_HARM,TRUE);
	} else {
...
void spell_cause_serious(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
	...
	if (global_damq){ 
    damage( ch, victim, dice(4, 16) + level / 2, sn,DAM_HARM,TRUE);
	} else {
...
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    victim->hit = UMIN( victim->hit + 100*0.10, victim->max_hit );
    update_pos( victim );
...
void spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
	...
	if ( in_obj->carried_by != NULL && can_see(ch,in_obj->carried_by))
	{
	    ...
	    || !global_gquest || !in_obj->carried_by->on_gquest)
	    {
		found = TRUE;
		number++;
