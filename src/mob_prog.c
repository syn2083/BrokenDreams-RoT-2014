...
#define CHK_HASSTR	(52)
#define CHK_HASWIS	(53)
#define CHK_HASINT	(54)
#define CHK_HASDEX	(55)
#define CHK_HASCON	(56)
...
const char * fn_keyword[] =
{
	...
	"str",					//check entity stat values
	"wis",				//check entity stat values
	"int",					//check entity stat values
	"dex",				//check entity stat values
	"con",				//check entity stat values
 ...
 */
bool has_item( CHAR_DATA *ch, unsigned long vnum, sh_int item_type, bool fWear )
{
    OBJ_DATA *obj;
 ...
 */
bool get_mob_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA *room, unsigned long vnum )
{
    CHAR_DATA *mob;
 ...
 */
bool get_obj_vnum_room( CHAR_DATA *ch, OBJ_DATA *obj, ROOM_INDEX_DATA *room, unsigned long vnum )
{
    OBJ_DATA *vobj;
 ...
 *----------------------------------------------------------------------
 */
int cmd_eval_mob( unsigned long vnum, char *line, int check,
...
int cmd_eval_obj( unsigned long vnum, char *line, int check,
...
int cmd_eval_room( unsigned long vnum, char *line, int check,
...
void program_flow( 
        unsigned long pvnum,  /* For diagnostic purposes */
...

{
    ...
    unsigned long mvnum = 0,ovnum = 0,rvnum = 0;
			...
			CHAR_DATA *ch, OBJ_DATA *dropped, int type )
{
	...
	for ( prg = obj->pIndexData->oprogs; prg; prg = prg->next )
	    if ( prg->trig_type == type )
	    {
		program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, (void *) obj, NULL );
		return;
	    }
    }
    else if ( room )
