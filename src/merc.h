...
/*
 * Uncomment the following line if compiling on a Solaris 2.5.1+ system.
 */
#ifndef WIN32
#define solaris
#endif
...
typedef	struct	drm_index_data		DRM_INDEX_DATA;
typedef	struct	dream_act_data		DREAM_ACT_DATA;
typedef	struct	dream_change_data	DREAM_CHANGE_DATA;
typedef	struct	dream_object_data	DREAM_OBJECT_DATA;
...
#define MAX_DUPES		   10
#define MAX_CLASS		   18
// #define MAX_CLASS		   21 35 60 for five new classes per tier
...
#define PULSE_SQL			( 300 * PULSE_PER_SECOND)
...
#define MCLT_1			( MAX_CLASS-9 )
#define MCLT_2			( MAX_CLASS-9 )
...
struct deity_data
{
    ...
    sh_int		race;
    sh_int		worshippers;
    unsigned long		vnum_obj;
    unsigned long		vnum_avatar;
    sh_int		cost_obj;
    sh_int		cost_avatar;
    sh_int		cost_corpse;
    sh_int		cost_recall;
...
#define CON_MUD_GET_NAME		17
#define CON_MUD_GET_BASE		18
#define CON_MUD_GET_ADDRESS		19
#define CON_MUD_GET_PORT		20
#define CON_MUD_SAVE			21
#define CON_GET_SECOND_CLASS		22
#define CON_REROLL			23
#define CON_COPYOVER_RECOVER    	24
#define CON_GET_WIZI      		25
...
struct	shop_data
{
    ...
    unsigned long	keeper;			/* Vnum of shop keeper mob	*/
...
struct	class_type
{
    ...
    char 		who_name	[6];	/* Five-letter name for 'who'	*/
    char 		swho_name	[3];	/* Two-letter name for 'who'	*/
...
struct race_type
{
    ...
    long		act;			/* act bits for the race */
 	long    	exbit1_flags;                 /* extra act bits for races */
...
struct pc_race_type  /* additional data for pc races */
{
    ...
    char *	home[MAX_HOME];		/* home towns for the race */
    bool		home_active;		/* home towns active for the race */
...
#define ACT2_IS_NPC		(A)		/* Auto set for mobs	*/
#define ACT2_FORGER 		(B)
...
#define OBJ_VNUM_LADDER		     56
#define OBJ_VNUM_QPOUCH		     57

#define OBJ_VNUM_ROSE		   1001

#define OBJ_VNUM_PIT		   3010
#define OBJ_VNUM_PIT_SCHOOL	   3723
#define OBJ_VNUM_PIT_NT		   9603
...
#define OVPKT5	  29105
#define OVPKT6	  29106
#define IMPTOKEN  29107
#define MG_WEAPON  29108
#define MG_ARMOR  29109
...
#define	ROOM_VNUM_MIDGAARD	   2900
#define ROOM_VNUM_MORGUE	   3000
#define ROOM_VNUM_TEMPLE	   3001
#define ROOM_VNUM_TEMPLEB	   3365
#define ROOM_VNUM_QUEST_RECALL	   3014
#define ROOM_VNUM_PIT		   3367
#define ROOM_VNUM_ALTAR		   3054
#define ROOM_VNUM_ALTARB	   3366
#define ROOM_VNUM_SCHOOL	   3700
#define ROOM_VNUM_MORGUE_SCHOOL	   3762
#define ROOM_VNUM_BALANCE	   4500
#define ROOM_VNUM_CIRCLE	   4400
#define ROOM_VNUM_DEMISE	   4201
#define ROOM_VNUM_HONOR		   4300
#define ROOM_VNUM_CHAIN		   7914
#define ROOM_VNUM_LADDER	   9828
#define ROOM_VNUM_CLANS		  20000

#define ROOM_VNUM_GOOD_NT	   9609
#define ROOM_VNUM_EVIL_NT	   9800
#define ROOM_VNUM_MORG_NT	   9802
#define ROOM_VNUM_AWINNER         27033
#define ROOM_VNUM_ALOSER          27034
...
{								\
  char swh_temp[255];						\
  if (!IS_NPC(_player) && IS_SET(_player->act, PLR_COLOUR))	\
  {								\
     swh_temp[0] = '\0';					\
...
/*Ok so, I wanted new color system, but to retain the old stuff, because I'm lazy, and alter colour is neat, so this is the OLD color system -Syn*/
/*
...
#define ONORMAL      0        /* Bright/Normal colours */
#define BRIGHT      1
#define ALTER_COLOUR( type )    if( !str_prefix( argument, "ORED" ) )        \
                {                        \
                    ch->pcdata->type[0] = ONORMAL;        \
                    ch->pcdata->type[1] = ORED;            \
                }                        \
                else if( !str_prefix( argument, "hi-ORED" ) )    \
                {                        \
                ...
                else if( !str_prefix( argument, "OYELLOW" ) )    \
                {                        \
                    ch->pcdata->type[0] = ONORMAL;        \
                    ch->pcdata->type[1] = OYELLOW;        \
                }                        \
                else if( !str_prefix( argument, "hi-OYELLOW" ) )    \
                {                        \
                    ch->pcdata->type[0] = BRIGHT;        \
                    ch->pcdata->type[1] = OYELLOW;        \
                }                        \
                ...
                else if( !str_prefix( argument, "OMAGENTA" ) )    \
                {                        \
                    ch->pcdata->type[0] = ONORMAL;        \
                    ch->pcdata->type[1] = OMAGENTA;        \
                }                        \
                else if( !str_prefix( argument, "hi-OMAGENTA" ) ) \
                {                        \
                    ch->pcdata->type[0] = BRIGHT;        \
                    ch->pcdata->type[1] = OMAGENTA;        \
                }                        \
                ...
                else if( !str_prefix( argument, "OWHITE" ) )    \
                {                        \
                    ch->pcdata->type[0] = ONORMAL;        \
                    ch->pcdata->type[1] = OWHITE;        \
                }                        \
                else if( !str_prefix( argument, "hi-OWHITE" ) )    \
                {                        \
                    ch->pcdata->type[0] = BRIGHT;        \
                    ch->pcdata->type[1] = OWHITE;        \
                }                        \
...
#define PLR_FREEZE		(Y)
#define PLR_TWIT		(Z)
#define PLR_LQUEST		(aa)
#define PLR_KEY			(bb)
#define PLR_NOCLAN		(cc)
#define PLR_SECOND		(dd)
#define PLR_THIRD		(ee)
#define PLR_ANSI		          16384 //N

/* new bit field exbit1 */
#define RECRUIT		(A)
#define PK_VETERAN		(B)
#define PK_KILLER		(C)
#define PK_LAWFUL		(D)
#define EXBIT1_CHALLENGED          (E)
#define EXBIT1_CHALLENGER          (F)
#define PLR_QUESTOR     (G)
#define PK_KILLER2		(H)
/*
#define PK_				()
#define PK_				()
...
/*
 * Prototype for a dream.
 */
struct  drm_index_data
{
    DRM_INDEX_DATA *			next;
    DREAM_ACT_DATA *			act;
    DREAM_CHANGE_DATA *	change;
    DREAM_OBJECT_DATA *	object;
    char *              					description;
    unsigned long              		vnum;
    sh_int              					move;
};
...
/*
 * Act data for a dream.
 */
struct dream_act_data
{
    DREAM_ACT_DATA *	next;
    char *		string;
};
...
/*
 * Change data for a dream.
 */ 
struct dream_change_data 
{
    DREAM_CHANGE_DATA *	next; 
    char *              keyword; 
    char *		value;
};

/*
 * Object data for a dream.
 */ 
struct dream_object_data 
{
    DREAM_OBJECT_DATA *	next; 
    char *		roomchar; 
    unsigned long		vnum;
    int			variable;
    int			t_min;
...
struct	area_data
{
    ...
    unsigned long			vnum;		/* OLC */ /* Area vnum  */
    int			area_flags;	/* OLC */
...
#define LOG_HELP			256
#define LOG_OLC		512
#define	LOG_UWID	1024
#define LOG_ALL      2047    // All the others added up
...
#define TAR_CHAR_OFF_NEXTROOM	    8
...
extern	sh_int	gsn_right_cross;
extern	sh_int	gsn_left_hook;
...
{                                          \
	...
	{                                       \
		...
		if (prev == NULL)                   \
		{                                   \
			log_string(LOG_BUG, #pdata " not found in " #list "."); \
		}                                   \
	}                                       \
} while(0)
...
#define IS_NPC2(ch)		(IS_SET((ch)->act2, ACT2_IS_NPC))
...
#define IS_QUESTOR(ch)          (IS_SET((ch)->exbit1_flags,PLR_QUESTOR))
...
extern	const	struct	wiznet_type	levnet_table	[];
extern	const	struct	attack_type	attack_table	[];
extern	const	struct	moblev_type	moblev_table	[MAX_LEVEL+1];
...
extern		bool			global_gquest;
extern		bool			global_xpq;
extern		bool			global_prq;
extern		bool			global_damq;
extern          sh_int                  arena;
...
extern long	char_count;

//Project infos - Syn
/* Project and Project Log Structs */
extern int mud_project_count;
extern int plog_count;

typedef struct mud_project_data MUD_PROJECT_DATA;
typedef struct plog_data PLOG_DATA;

void alloc_mud_projects args((void));
void alloc_plogs args((void));

extern MUD_PROJECT_DATA **mud_project_table;
extern PLOG_DATA **plog_table;

enum {PROJ_COL_ID, PROJ_COL_CB, PROJ_COL_TY, PROJ_COL_AT, PROJ_COL_PN, PROJ_COL_PD, PROJ_COL_CO, PROJ_COL_LC, PROJ_COL_PS, PROJ_COL_PC, PROJ_COL_FO};
enum {PLOG_COL_PNA, PLOG_COL_ID, PLOG_COL_CB, PLOG_COL_CO, PLOG_COL_LS, PLOG_COL_LM};

struct mud_project_data
{
int project_id;
char *created_by;
char *type;
...
// void	show_warning	args( ( CHAR_DATA *ch ) );

/* act_move.c */
void	move_char	args( ( CHAR_DATA *ch, int door, bool follow, bool quiet ) );
void	move_char_transfer	args( ( CHAR_DATA *ch, bool quiet ) );
...
/* arena stuff */
void do_challenger      args( ( CHAR_DATA *ch, char *argument) );
...
void	exbit1		args( ( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type ) );
...
MID *	get_mob_index	args( ( unsigned long vnum ) );
OID *	get_obj_index	args( ( unsigned long vnum ) );
RID *	get_room_index	args( ( unsigned long vnum ) );
DID *	get_drm_index	args( ( unsigned long vnum ) );
PC *    get_prog_index args( ( unsigned long vnum, int type ) );
...
void *	alloc_mem	args( ( int iAmount ) );
void *	alloc_perm	args( ( int sMem ) );
void	free_mem	args( ( void *ptr, int size ) );
...
long	levnet_lookup	args( ( const char *name) );
...
int	pk_range	args( (CHAR_DATA *ch, CHAR_DATA *victim));
bool	pk_rangerev	args( (CHAR_DATA *ch, CHAR_DATA *victim));
...
CD *	get_exact_world	args( ( CHAR_DATA *ch, char *argument ) );
...
bool	can_see_door	args( ( CHAR_DATA *ch, long exit_info ) );
bool	can_drop_obj	args( ( CHAR_DATA *ch, OBJ_DATA *obj ) );
char *	item_type_name	args( ( OBJ_INDEX_DATA *obj ) );
...
char *	exbit1_bit_name	args( ( int exbit1_flags ) );
...
/* mob_prog.c */
void	program_flow	args( ( unsigned long vnum, char *source, CHAR_DATA *mob, 
...
void	append_mud	args( ( MUD_DATA *pmud ) );
...
int     spell_avail_class     args( ( CHAR_DATA *ch, const char *name) );
int     spell_avail_clasb     args( ( CHAR_DATA *ch, const char *name) );

/* special.c */
SF *	spec_lookup	args( ( const char *name ) );
char *	spec_name	args( ( SPEC_FUN *function ) );
/* int declarations and misc */
int bllmax, crbmax, crnmax, srpmax, mngmax; 
int mnkmax, swlmax, mntmax, drgmax, tgrmax;
int bllsty, srpsty, crbsty, crnsty, mngsty; 
int mnksty, swlsty, mntsty, drgsty, tgrsty;
int bllmin, srpmin, crbmin, crnmin, mngmin; 
int mnkmin, swlmin, mntmin, drgmin, tgrmin;
int chbllsty, chsrpsty, chcrbsty, chcrnsty, chmngsty; 
int chmnksty, chswlsty, chmntsty, chdrgsty, chtgrsty;

/* teleport.c */
RID *	room_by_name	args( ( char *target, int level, bool error) );

/* update.c */
void	change_level	args( ( CHAR_DATA *ch ) );
void	change_level_quiet	args( ( CHAR_DATA *ch ) );
...
extern		int			top_area;
extern		int			top_ed;
extern		unsigned long			top_exit;
extern		int			top_help;
extern		unsigned long			top_mob_index;
extern		unsigned long			top_obj_index;
extern		unsigned long			top_reset;
extern		unsigned long			top_room;
extern		unsigned long			top_shop;
extern 		int 							nAllocPerm;
extern		int							nAllocString;
 
extern		unsigned long			top_vnum_mob;
extern		unsigned long			top_vnum_obj;
extern		unsigned long			top_vnum_room;
