...
#define new_buf(size) __new_buf(size, __FILE__, __FUNCTION__, __LINE__)
#define add_buf(buffer, text) __add_buf(buffer, text, __FILE__, __FUNCTION__, __LINE__)
BUFFER	*__new_buf args( (int size, const char *file, const char *function, int line) );
int BufPrintf args(( BUFFER * buffer, char * fmt, ... ));
BUFFER  *new_buf_size args( (int size) );
void	free_buf args( (BUFFER *buffer) );
bool	add_buf args( (BUFFER *buffer, const char *text, const char *file, const char *function, unsigned line) );
