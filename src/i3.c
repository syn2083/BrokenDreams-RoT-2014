...
void I3_loop( void )
{
   ...
   {
		int len = MSL;
		ioctl(I3_socket, FIONREAD, &len);
		if (len > 0) {
		  ret = recv(I3_socket, I3_input_buffer + I3_input_pointer, len,0);
		}
      ...
      if( !ret || ( ret < 0 && errno != EAGAIN && errno != EWOULDBLOCK ) )
      {
         ...
         if( ret < 0 )
		 {
			...
			log_string(LOG_CRIT,"i3 error len %d ret %d", len, ret);
		}
         else
		 {
