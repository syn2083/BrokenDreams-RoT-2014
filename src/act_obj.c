	...
	else if (gold != 0)
	{
	    ...
	    /*
            ...
            if ( IS_NPC(victim) && HAS_TRIGGER_MOB( victim, TRIG_BRIBE ) )
                p_bribe_trigger( victim, ch, amount * 100 * 100 );
	    /*
             * Bribe trigger
             */
            if ( IS_NPC(victim) && HAS_TRIGGER_MOB( victim, TRIG_BRIBE ) )
                p_bribe_trigger( victim, ch, amount * 100 );
	    /*
             * Bribe trigger
             */
            if ( IS_NPC(victim) && HAS_TRIGGER_MOB( victim, TRIG_BRIBE ) )
...
void do_recite( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( ( victim = get_char_room(ch, NULL, arg2) ) == NULL &&   ( obj    = get_obj_here  ( ch, NULL, arg2 ) ) == NULL )
	{
	    send_to_char( "You can't find it.\n\r", ch );
	    return;
	}
