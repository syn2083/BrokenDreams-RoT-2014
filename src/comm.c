...
void	read_from_buffers	args( ( DESCRIPTOR_DATA *d ) );
...
bool read_from_descriptor( DESCRIPTOR_DATA *d )
{
	...
	{
	    log_string(LOG_GAME, "EOF encountered on read." );
	    return FALSE;
	}
#if !defined( AmigaTCP ) && !defined( WIN32 )
...
void bust_a_prompt (CHAR_DATA * ch)
{
    ...
    while (*str != '\0')
    {
        ...
        switch (*str)
        {
            ...
            case 'h': sprintf (buf2, "#R%d#n", ch->hit); i = buf2; break;
            case 'q': 

if ( global_prq && global_xpq && global_damq    ){ sprintf (buf2, "PRQx2 XPx2 DAMx2"); i = buf2; break; }
else if ( !global_prq && !global_xpq && !global_damq ){ sprintf (buf2, "off"); ADD_COLOUR(ch, buf2, L_CYAN); i = buf2; break; }
...
void write_to_buffer(DESCRIPTOR_DATA * d, const char *txt, int length)
	...
	if (length >= MAX_STRING_LENGTH)
	{
		...
		log_string(LOG_BUG, "Too much text to show, clearing..... (%s)",d->character->name);
		sprintf(tb,
			"Too much text to show, clearing....");
	length = strlen(tb);
	 log_string(LOG_BUG, "Too much text to show, clearing..... (%s)",d->character->name);
	return;*/
		log_string(LOG_BUG, "Write_to_buffer: Way too big. Closing.");
		d->outtop = 0;
		d->outsize = 0;
		return;
	}
	...
	if (d->character == NULL || (d->connected != CON_PLAYING)) 
		...
		ansi = (IS_SET(d->character->act, PLR_COLOUR)) ? TRUE : FALSE;
...
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = 'm'; break; case 'X':
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = 'm'; break; case 'x':
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = 'm'; break; case 'w':
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = ';'; tb[j++] = ccode; tb[j++] = '7'; tb[j++] = 'm'; break;case 'V':
...
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = ';'; tb[j++] = ccode; tb[j++] = '4'; tb[j++] = 'm'; break; case 'm':
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = ';'; tb[j++] = ccode; tb[j++] = '5'; tb[j++] = 'm'; break; case 'p':
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '0'; tb[j++] = ';'; tb[j++] = ccode; tb[j++] = '5'; tb[j++] = 'm'; break; case 'P':
if (!ansi) continue; tb[j++] = 27; tb[j++] = '['; tb[j++] = '1'; tb[j++] = ';'; tb[j++] = ccode; tb[j++] = '5'; tb[j++] = 'm'; break; case 'M':
...
d->outsize          	= 15999;
d->outtop       		= 0;
write_to_buffer(d, " #0[#CS#cp#Ca#cm #RC#rh#Ro#rk#Re#0]#n\n\r", 0);
...
while ( d->outtop + length >= d->outsize )
{
	...
	if (d->outsize >= 32000)
	{
		write_to_buffer(d, "\n\r#0[#CSsss#cp#Ca#cm #RC#rh#Ro#rk#Re#0]#n\n\r",0);
		d->outsize = 15999;
	}                        
	outbuf = calloc(1, sizeof(char *) * 2 * d->outsize);
 ...
 */
 //Added in some detection for EAGAIN, and the ability to loop back, and resend data that was 'blocked' by a full buffer.. no more drops from socket saturation!
bool write_to_descriptor (int desc, char *txt, int length)
{
	...
	for ( iStart = 0; iStart < length;  )
	{
		...
		#else
		if ( ( nWrite = send( desc, txt + iStart, nBlock , MSG_NOWAIT) ) < 0 )
		#endif
		{
			if(nWrite < 0)
			{
...
*/
	...
	else
	{
	    ...
	    sprintf( buf, "NOTICE: `Broken Dreams` REQUIRES role-playing type names, If your\n\r");
	    write_to_buffer( d, buf, 0 );
...
case CON_GET_OLD_PASSWORD:
	...
	sprintf( log_buf, "%s@%s has joined Broken Dreams", ch->name, d->host );
	log_string(LOG_GAME, log_buf );
...
case CON_GET_NEW_CLASS:
	...
	if(ch->ud->uwid)
		free(ch->ud->uwid);
		char_count++;
	ch->ud->uwid = char_uwid_constructor(ch);
	ch->ud->type = U_PLR;
...
case CON_DEFAULT_CHOICE:
	write_to_buffer(d,"\n\r",2);
	    ...
	    unboxed_help(ch,"group header");
	    list_group_costs(ch);
	    write_to_buffer(d,"You already have the following skills:\n\r",0);
	    do_skills(ch,"");
	    do_bskills(ch,"");
	    unboxed_help(ch,"menu choice");
...
void page_to_char_bw( const char *txt, CHAR_DATA *ch )
{
    ...
    ch->desc->showstr_head = alloc_mem(sizeof(txt) + 1);
...
//Page_to_char modified to use Buffer System
//Also removed all the color garbage as that is handled by write_to_buffer now
//Sure we are buffering a buffer, but it works great.
//Syn	
void page_to_char (const char *txt, CHAR_DATA * ch)
{
	...
	add_buf(buffer1, txt);
    ch->desc->showstr_point = buffer1->data;
    show_string(ch->desc,"");
	free_buf(buffer1);
}
    ...
    for (; to != NULL; to = to->next_in_room)
    {
       ...
       //colourconv (pbuff, buf, to);
		if (to->desc && (to->desc->connected == CON_PLAYING))
			write_to_buffer( to->desc, buf, point - buf); /* changed to buffer to reflect prev. fix */
        else if (MOBtrigger)
