...
#if defined( VERBOSE )
			fprintf( fp, "D 0 %lu %lu %d The %s door of %s is %s\n", 
				pRoomIndex->vnum,
				pExit->orig_door,
...
void save_resets( FILE *fp, AREA_DATA *pArea )
{
	...
	case 'M':
	    ...
	    fprintf( fp, "M 0 %lu %d %d %d Load %s\n", 
	        pReset->arg1,
                pReset->arg2,
                pReset->arg3,
		pReset->arg4,
	...
	case 'O':
	    ...
	    fprintf( fp, "O 0 %lu 0 %d %s loaded to %s\n", 
	        pReset->arg1,
                pReset->arg3,
	...
	case 'P':
            pLastObj = get_obj_index( pReset->arg1 );
	    fprintf( fp, "P 0 %lu %d %d %d %s put inside %s\n", 
	        pReset->arg1,
	        pReset->arg2,
                pReset->arg3,
                pReset->arg4,
	...
	case 'G':
	    fprintf( fp, "G 0 %lu 0 %s is given to %s\n",
	        pReset->arg1,
	        capitalize(get_obj_index( pReset->arg1 )->short_descr),
	...
	case 'E':
	    fprintf( fp, "E 0 %lu 0 %d %s is loaded %s of %s\n",
	        pReset->arg1,
                pReset->arg3,
                capitalize(get_obj_index( pReset->arg1 )->short_descr),
	...
	case 'R':
	    ...
	    fprintf( fp, "R 0 %lu %d Randomize %s\n", 
	        pReset->arg1,
                pReset->arg2,
                pRoom->name );
            break;
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pMobIndex = mob_index_hash[iHash]; pMobIndex; pMobIndex = pMobIndex->next )
        {
            if ( pMobIndex && pMobIndex->area == pArea && pMobIndex->pShop )
            {
                ...
                fprintf( fp, "%lu ", pShopIndex->keeper );
